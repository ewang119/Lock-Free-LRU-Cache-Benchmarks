...
flck::atomic<int> total_ins;
flck::atomic<int> total_rem;
...
struct LfHashMap {
  ...
  // flck::memory_pool<ssize_t> size_pool;
  ...
  int capacity(Table& table){
    // printf("total ins rem %d %d\n", total_ins.load(), total_rem.load());
    return total_ins.load() - total_rem.load();
  }
    ...
    while (true) {
      ...
      if (s->try_lock([=] {
        ...
        *cur = node_pool.new_obj(k, v, nullptr);
        s->version_num = vn + 1;
        total_ins.cam(total_ins.load(), total_ins.load() + 1);
        return true;
      }))
        return true;
    }
    ...
    while (true) {
      ...
      if (s->try_lock([=] {
        ...
        *cur = nxt->next.load();
        ...
        total_rem.cam(total_rem.load(), total_rem.load() + 1);
        s->version_num = vn + 1;
        return true;
      }))
        return true;
